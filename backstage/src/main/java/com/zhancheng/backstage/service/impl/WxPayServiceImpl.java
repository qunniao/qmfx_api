...
public class WxPayServiceImpl implements WxPayService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public String wxPayRefundUnifiedNotify(String notifyXml) {

        synchronized (new Object()) {
            try {
                Map<String, String> notify = WXPayUtil.xmlToMap(notifyXml);
                String outRefundNo = notify.get("out_refund_no");
                String refundFee = notify.get("refund_fee");

                OrderRefund orderRefund = orderRefundMapper.selectById(outRefundNo);

                //判断订单号是否正确
                if (ObjectUtil.isNull(orderRefund)) {
                    return "<xml>" + "<return_code><![CDATA[FAIL]]></return_code>" + "<return_msg><![CDATA[退款订单不存在]]></return_msg>" + "</xml>";
                }
                log.info("*****3*****");

                Order order = orderMapper.selectById(orderRefund.getOrderId());
                //判断订单是否是待付款状态(订单状态：-1.已删除0.已关闭1.待确认2.待付款3.待总部处理4.待发货5.待收货6.待评价7.已完成)
                if (ObjectUtil.isNull(order.getOrderState()) || order.getOrderState() != OrderStateEnum.REFUNDING.getState()) {
                    return "<xml>" + "<return_code><![CDATA[FAIL]]></return_code>" + "<return_msg><![CDATA[订单状态异常,可能不是待退款]]></return_msg>" + "</xml>";
                }
                log.info("*****4*****");
                // 数据库查出来的金钱
                int intValue = orderRefund.getRefundMoney().multiply(new BigDecimal("100")).intValue();
                //判断金额是否一致
                if (!refundFee.equals(String.valueOf(intValue))) {
                    return "<xml>" + "<return_code><![CDATA[FAIL]]></return_code>" + "<return_msg><![CDATA[退款金额不一致]]></return_msg>" + "</xml>";
                }
                log.info("*****5*****");
                //判断返回码是否是SUCCESS
                if ("SUCCESS".equals(notify.get("return_code"))) {
                    ...
                    if (WXPayUtil.isSignatureValid(notify, wxConfig.getKey())) {
                        ...
                        order.setOrderState(OrderStateEnum.REFUND_CCOMPLETE.getState());
                        order.updateById();
                ...
                }else if("FAIL".equals(notify.get("return_code"))){
                    //退款失败
                    orderRefund.setRefundState(Constant.RefundStateFail);
